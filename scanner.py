...
class CVEExploiter:
    def __init__(self, verbose=False, output_file=None, threads=10, timeout=5, exploit=False, enrich=False):
    ...
    # ═══════════════════════════════════════════════════
    #  ENRICHMENT MODULES (v3.0+)
    # ═══════════════════════════════════════════════════

    def _run_enrichment(self, host):
        """
        Run all enrichment modules against a host.
        Called when --enrich flag is set.
        """
        enrichment = {}
        ...
        try:
            ...
            if self.verbose:
                print(f"  [banner_grabber] {e}")

        try:
            from modules.cloud_enrich import enrich_ip
            # Enrich the target IP
            ip_info = enrich_ip(host, timeout=self.timeout)
            if ip_info:
                enrichment["ip_intel"] = ip_info
            # Also enrich known Flock cloud IPs if we detected them
            cloud_ips = []
            for finding in getattr(self, '_current_findings', []):
                if 'cloud' in finding.get('type','').lower() or 'flock' in finding.get('detail','').lower():
                    # We'd need to extract IPs — handled at print time
                    pass
        except Exception as e:
            if self.verbose:
                print(f"  [cloud_enrich] {e}")

        # Telemetry from admin web page
        http_res = None
        for port in (80, 443):
            try:
                body = self._http_get(host, "/", port=port)
                if body:
                    from modules.telemetry import extract_telemetry
                    telemetry = extract_telemetry(body)
                    if telemetry:
                        enrichment[f"telemetry_{port}"] = telemetry
                    break
            except Exception:
                pass
        ...
        try:
            ...
            adb_data = adb_deep_collect(host, timeout=self.timeout)
            if adb_data:
                enrichment["adb_deep"] = adb_data
        except Exception as e:
            if self.verbose:
                print(f"  [adb_deep] {e}")

        # Network map (passive, ARP-based)
        try:
            from modules.network_map import NetworkMapper
            nm = NetworkMapper(host, timeout=self.timeout)
            net_map = nm.full_map()
            if net_map and net_map.get("discovered_devices"):
                enrichment["network_map"] = net_map
        except Exception as e:
            if self.verbose:
                print(f"  [network_map] {e}")

        # ── NEW v3.1 modules ──

        # Credential extraction from HTTP bodies
        try:
            from modules.creds_extractor import extract_creds, extract_creds_from_headers
            creds = []
            for port in (80, 443):
                body = self._http_get(host, "/", port=port)
                if body:
                    found = extract_creds(body, source=f"admin_page_{port}")
                    # Also try common config endpoints
                    for cfg_path in ("/config", "/config.json", "/api/config", "/status", "/metadata"):
                        more_body = self._http_get(host, cfg_path, port=port)
                        if more_body:
                            found += extract_creds(more_body, source=f"{cfg_path}_{port}")
                    if found:
                        creds.extend(found)
            if creds:
                enrichment["credentials"] = creds
                # Write to file
                try:
                    from modules.creds_extractor import write_creds_report
                    creds_path = os.path.join(
                        os.path.dirname(self.output_file or "results"),
                        f"creds_{host.replace('.', '_')}.txt"
                    ) if self.output_file else f"creds_{host.replace('.', '_')}.txt"
                    write_creds_report(creds, creds_path)
                except Exception as e:
                    if self.verbose:
                        print(f"  [creds write] {e}")
        except Exception as e:
            if self.verbose:
                print(f"  [creds_extractor] {e}")

        # Prometheus scraping
        try:
            from modules.prometheus_scraper import probe_prometheus, scan_subnet_for_prometheus
            # Get gateway info from network_map or adb_deep
            gateway = None
            if enrichment.get("adb_deep"):
                gateway = enrichment["adb_deep"].get("gateway")
            elif enrichment.get("network_map", {}).get("camera", {}).get("network_info"):
                gateway = enrichment["network_map"]["camera"]["network_info"].get("gateway")

            prom_results = []
            # Probe the camera host itself
            p = probe_prometheus(host, timeout=self.timeout)
            if p:
                prom_results.append(p)
            # Probe gateway if available
            if gateway:
                p = probe_prometheus(gateway, timeout=self.timeout)
                if p:
                    prom_results.append(p)
            if prom_results:
                enrichment["prometheus"] = prom_results
        except Exception as e:
            if self.verbose:
                print(f"  [prometheus] {e}")

        # S3 URL capture from HTTP bodies
        try:
            from modules.s3_url_catcher import scan_traffic_for_s3
            http_bodies = []
            for port in (80, 443):
                for path in ("/", "/config", "/status", "/metadata", "/api/v1/status"):
                    body = self._http_get(host, path, port=port)
                    if body:
                        http_bodies.append({
                            "body": body,
                            "source": f"{host}:{port}{path}",
                        })
            s3_urls = scan_traffic_for_s3(http_responses=http_bodies)
            if s3_urls:
                enrichment["s3_urls"] = s3_urls
                try:
                    from modules.s3_url_catcher import write_s3_report
                    s3_path = os.path.join(
                        os.path.dirname(self.output_file or "results"),
                        f"s3_urls_{host.replace('.', '_')}.txt"
                    ) if self.output_file else f"s3_urls_{host.replace('.', '_')}.txt"
                    write_s3_report(s3_urls, s3_path)
                except Exception as e:
                    if self.verbose:
                        print(f"  [s3 write] {e}")
        except Exception as e:
            if self.verbose:
                print(f"  [s3_url_catcher] {e}")
    ...
    def discover_fingerprints(self, host):
        ...
        # 7) Enrichment (v3.0+ — banners, cloud provider, telemetry, ADB deep, network map)
        if self.enrich:
            self._current_findings = findings
            enrich_data = self._run_enrichment(host)
            if enrich_data:
                findings.append({
                    "type": "ENRICHMENT",
                    "detail": f"Enrichment data collected: {', '.join(k for k in enrich_data.keys())}",
...
def main():
    ...
    parser.add_argument('--enrich', action='store_true', help='Enable enrichment modules (banners, cloud provider, telemetry, ADB deep, network map)')
